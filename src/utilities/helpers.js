...
export function doubleFilterByTitleAndCategory(arr, title, categories) {
  ...
  if (categories === "all" && title !== "") {
    ...
    if (title === "") {
      ...
      filteredArr = filteredArr.filter(e =>
        satisfiesCategories(categories, e.categories)
      );
    } else {
...
export const createBootstrapDropDownItems = elements => {
  return elements.map(e => {
    return (
      <Dropdown.Item key={Math.random()} as="button" eventKey={e}>
        {e}
      </Dropdown.Item>
    );
  });
...
export const mapSortPropertyToSortLabel = sortProperty => {
  ...
  else if (sortProperty === "last_added_to_library")
    return "Add To Library Date";
  else return "Title";
};
...
export const calculatePercentOfChapterForCurrentPage = (
  chapterContent,
  lastCharOnPageIndex
) => {
  return Math.round(
    ((1.0 * lastCharOnPageIndex) / chapterContent.length) * 100
  );
};
